import { $el, MikiDialog } from "./ui.js";
...
class MikiLoggingDialog extends MikiDialog {
	constructor(logging) {
		super();
	...
	export() {
		...
		const a = $el("a", {
			href: url,
			download: `Mikiui-logs-${Date.now()}.json`,
			style: { display: "none" },
			parent: document.body,
		});
		a.click();
	...
	show(entries) {
		...
		const headers = Object.values(cols).map((title) =>
			$el("div.Miki-logging-title", {
				textContent: title,
			})
		);
		const rows = entries.map((entry, i) => {
			return $el(
				"div.Miki-logging-log",
				{
...
export class MikiLogging {
	...
	constructor(app) {
		...
		this.dialog = new MikiLoggingDialog(this);
		this.addSetting();
		this.catchUnhandled();
		this.addInitData();
	}
	...
	addSetting() {
		const settingId = "Miki.Logging.Enabled";
