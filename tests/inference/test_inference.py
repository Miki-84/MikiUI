...
from Miki.samplers import KSampler
...
DEFAULT_Miki_GRAPH = MikiGraph(graph=default_graph, sampler_nodes=['10','14'])
DEFAULT_Miki_GRAPH_ID = os.path.splitext(os.path.basename(default_graph_file))[0]

#
# Loop through these variables
#
Miki_graph_list = [DEFAULT_Miki_GRAPH]
Miki_graph_ids = [DEFAULT_Miki_GRAPH_ID]
prompt_list = [
    'a painting of a cat',
]
...
class TestInference:
    ...
    def start_client(self, listen:str, port:int):
        # Start client
        Miki_client = MikiClient()
        # Connect to server (with retries)
        n_tries = 5
        for i in range(n_tries):
            time.sleep(4)
            try:
                Miki_client.connect(listen=listen, port=port)
    ...
    @fixture(scope="class", params=Miki_graph_list, ids=Miki_graph_ids, autouse=True)
    def _client_graph(self, request, args_pytest, _server) -> (MikiClient, MikiGraph):
        Miki_graph = request.param
        
        # Start client
        Miki_client = self.start_client(args_pytest["listen"], args_pytest["port"])

        # Warm up pipeline
        Miki_client.get_images(graph=Miki_graph.graph, save=False)

        yield Miki_client, Miki_graph
        del Miki_client
        del Miki_graph
        torch.cuda.empty_cache()
    ...
    @fixture
    def Miki_graph(self, _client_graph):
    ...
    def test_Miki(
        ...
        Miki_graph.set_filename_prefix(test_info)
        # Settings for Miki graph
        Miki_graph.set_sampler_name(sampler)
        Miki_graph.set_scheduler(scheduler)
        Miki_graph.set_prompt(prompt)

        # Generate
        images = client.get_images(Miki_graph.graph)
